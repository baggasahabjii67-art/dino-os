...
const defaultSettings = { theme: 'Dark', layout: 'Comfortable', accent: 'Purple', fontSize: 'Medium', readingDirection: 'Vertical', pageFit: 'Width', resolution: 'Auto', optimization: 'Balanced', preload: 'Enabled', animations: 'Enabled' };
...
const settingOptions = { theme: ['Dark', 'Light', 'Midnight'], layout: ['Compact', 'Comfortable', 'Spacious'], accent: ['Purple', 'Blue', 'Green', 'Orange'], fontSize: ['Small', 'Medium', 'Large'], readingDirection: ['Vertical', 'Horizontal'], pageFit: ['Width', 'Height', 'Original'], resolution: ['Auto', '720p', '1080p', '1440p'], optimization: ['Battery saver', 'Balanced', 'Quality'], preload: ['Enabled', 'Disabled'], animations: ['Enabled', 'Disabled'] };
...
function openReader(manga) {
  const modal = document.createElement('div'); modal.className = `reader-modal reader-${settings.readingDirection.toLowerCase()}`;
  modal.innerHTML = `<div class="reader-box" role="dialog" aria-modal="true"><button class="close" type="button" aria-label="Close reader">×</button><p class="muted">${escapeHtml(manga.title)}</p><h1>Chapter 1</h1><div class="reader-page reader-fit-${settings.pageFit.toLowerCase()} reader-resolution-${settings.resolution.toLowerCase()}" data-preload="${settings.preload.toLowerCase()}"><img src="${manga.cover}" alt="${escapeHtml(manga.title)} cover" /><p>Chapter content is not available from this source yet.</p><small>Direction: ${settings.readingDirection} · Fit: ${settings.pageFit} · Resolution: ${settings.resolution}</small></div></div>`;
